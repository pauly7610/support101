...
class ActivityGraph:
    ...
    def __init__(
        ...
        self._db_url = (
            database_url or os.getenv("AGENT_FRAMEWORK_DATABASE_URL") or os.getenv("DATABASE_URL")
        )
        self._engine = None
        ...
        try:
            ...
            async with self._session_factory() as session:
                try:
                    ...
                    logger.info(
                        "ActivityGraph: Apache AGE initialized (graph=%s)",
                        self._graph_name,
                    )
                except Exception as e:
                    logger.info(
                        "ActivityGraph: AGE not available (%s), using relational fallback",
                        e,
                    )
                    self._age_available = False
    ...
    async def get_customer_journey(
        ...
        journey = []
        customer_tickets = [
            e for e in self._edges if e["label"] == "FILED" and e["from_id"] == customer_id
        ]
    ...
    async def get_resolution_stats(self, tenant_id: str = "") -> dict[str, Any]:
        ...
        for key in self._nodes:
            label = key.split(":")[0]
            label_counts[label.lower() + "_count"] = (
                label_counts.get(label.lower() + "_count", 0) + 1
            )
