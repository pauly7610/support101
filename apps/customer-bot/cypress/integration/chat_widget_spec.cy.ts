...
describe('Customer Chat Widget', () => {
  ...
  it('shows unread badge when minimized and new message arrives', () => {
    cy.get('[aria-label="Minimize chat"]').click();
    cy.window().then(win => {
      win.dispatchEvent(new Event('focus'));
    });
    cy.get('textarea[aria-label="Chat input"]').type('Hello{enter}');
    cy.get('[aria-label="Open chat"] .bg-red-500').should('exist');
  });

  it('shows escalation badge and reports to backend', () => {
    cy.intercept('POST', '**/report_escalation').as('reportEscalation');
    cy.get('textarea[aria-label="Chat input"]').type('urgent problem{enter}');
  ...
  it('uploads and displays an image', () => {
    ...
    cy.fixture(fileName, 'base64').then(fileContent => {
      cy.get('label[for="file-upload"]').then(label => {
        cy.get('input[type="file"]').attachFile({ fileContent, fileName, mimeType: 'image/png', encoding: 'base64' });
      });
    });
  ...
  it('can react to a message with a like', () => {
    cy.get('textarea[aria-label="Chat input"]').type('Hello!{enter}');
    cy.get('button[aria-label="Like message"]').first().click();
    cy.get('button[aria-label="Like message"]').first().contains('👍');
  });

  it('shows typing indicator when agent is responding', () => {
    cy.get('textarea[aria-label="Chat input"]').type('Hi there!{enter}');
