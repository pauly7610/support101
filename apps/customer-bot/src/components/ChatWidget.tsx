...
export default function ChatWidget() {
  ...
  useEffect(() => {
    if (chatEndRef.current) (chatEndRef.current as any).scrollIntoView({ behavior: "smooth" });
  }, [messages]);
  ...
  function handleSend(e: React.FormEvent) {
    ...
    setMessages(prev => {
      // Simulate agent/LLM response after short delay, with error simulation
      setTimeout(() => {
        ...
        if (Math.random() < 0.1) {
          ...
          setErrorIdx(prevMsgs => prevMsgs ? prevMsgs.length : null);
        } else {
          const agentReply = {
            sender: "agent",
            text: `Thank you for your message${sentimentResult === 'urgent' ? ', we are escalating this to support.' : '!'}`,
            timestamp: Date.now() + 1000,
            sentiment: "normal"
          };
  ...
  return (
    <>
      <div
        className={`w-full max-w-md mx-auto border rounded shadow flex flex-col h-[500px] ${theme === 'dark' ? 'bg-gray-900 text-gray-100' : 'bg-white'}`}
      ...
      >
        <header className={`px-4 py-3 rounded-t flex items-center justify-between ${theme === 'dark' ? 'bg-gray-800 text-gray-100' : 'bg-primary-blue text-white'}`}>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              style={{ background: theme === 'dark' ? '#222' : '#e6f0ff', color: theme === 'dark' ? '#fff' : '#222', border: 'none' }}
            >
              {theme === 'dark' ? '🌙' : '☀️'}
            </button>
            <button
        ...
        <main className="flex-1 overflow-y-auto px-4 py-2" aria-live="polite">
          {messages.map((msg, i) => (
            <div
              key={i}
              className={`mb-2 flex items-end gap-2 ${msg.sender === "user" ? "justify-end" : "justify-start"} animate-fadein-slideup`}
            ...
            >
              ...
              <span
                className={`inline-block px-3 py-2 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-400 ${msg.sender === "user" ? (theme === 'dark' ? "bg-blue-900 text-blue-100" : "bg-blue-100 text-blue-900") : (theme === 'dark' ? "bg-gray-800 text-gray-100" : "bg-gray-100 text-gray-800")}`}
                style={{ color: theme === 'dark' ? '#fff' : '#1a1a1a', background: msg.sender === "user" ? (theme === 'dark' ? '#1e40af' : '#e6f0ff') : (theme === 'dark' ? '#222' : '#f4f4f4'), minWidth: 40, maxWidth: 240, wordBreak: 'break-word', border: msg.sentiment === 'urgent' ? '2px solid #ff4d4f' : undefined, boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}
                tabIndex={0}
              >
                {msg.text}
        ...
        <form className="p-3 flex gap-2 border-t" onSubmit={handleSend}>
          ...
          <textarea
            ...
            style={{ color: theme === 'dark' ? '#fff' : '#1a1a1a', background: theme === 'dark' ? '#222' : '#fff', borderColor: '#ccc', minHeight: 36, maxHeight: 90, resize: 'vertical' }}
