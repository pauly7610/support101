...
function groupByDay(escalations: any[]) {
...
export default function EscalationCharts() {
  ...
  useEffect(() => {
    idb.get(ESCALATION_LOG_KEY).then((log: any[] = []) => setEscalations(log));
  }, []);
      ...
      <div className="flex items-end gap-2 h-40">
        {days.map((day, i) => (
          <div key={day} className="flex flex-col items-center" style={{ width: 36 }}>
            <div style={{ height: `${(values[i] / max) * 120}px`, background: '#2563eb', width: 24, borderRadius: 4 }} title={`${values[i]} escalation${values[i] !== 1 ? 's' : ''}`}></div>
            <span className="text-xs mt-1" style={{ writingMode: 'vertical-lr', textOrientation: 'mixed', color: '#555' }}>{day}</span>
          </div>
        ))}
      </div>
    </div>
