...
class TestRAGChain:
    ...
    @pytest.fixture
    def rag(self):
        with patch('packages.llm_engine.chains.rag_chain.get_fastembed_model', return_value=AsyncMock()), \
             patch('packages.llm_engine.chains.rag_chain.ChatOpenAI', return_value=AsyncMock()):
            yield RAGChain()

    @patch('packages.llm_engine.chains.rag_chain.RAGChain._safe_query_pinecone', new_callable=AsyncMock)
    ...
    async def test_citation_filtering(self, mock_llm, mock_query, rag):
        ...
        mock_query.return_value = [
            {'score': SIMILARITY_THRESHOLD + 0.01, 'metadata': {'source_url': 'url1', 'text': 'Relevant doc', 'last_updated': '2025-01-01'}},
            {'score': SIMILARITY_THRESHOLD - 0.1, 'metadata': {'source_url': 'url2', 'text': 'Irrelevant', 'last_updated': '2025-01-01'}},
        ]
